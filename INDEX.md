...
## 🚨 運用方針（2026-04-09 夜・自己分析後）

本人の状況を元に、以下の制約が明らかになった:
1. **本人の作業時間**: 週 2 時間のみ（`research/scope-revision-plan.md`）
...
- [risk-register.md](research/risk-register.md) — リスク台帳（21 リスク、R-018〜R-021 を追加）
...
- [ ] **DEL フレーミング（LLM を認識エージェントとして扱う）を承認するか？**（最優先）
- [ ] S5 系と KD45 系のどちらを理論的ベースラインとすべきか？
...
1. **仮テーマ**: LLMの自己評価能力検証（verbalized confidence calibration）
   ...
   - **DEL フレーミング**: 動的認識論理の実証実験として位置づけ（教員承認待ち）
