...
（オプション）⑪ 効果の内訳を分離する視点もある & Brier Scoreは較正誤差(REL)と識別能力(RES)に分解できる。本実験データが十分なら発展させる & Bröcker 2009; Pohle 2020（探索的オプション，§5.2参照） \\
\bottomrule
\end{longtable}
...
知識 & JMMLU（公民・地理・慣用句の3科目，歴史は除外） & 250 & 4択MC & 選択肢一致 \\
...
採用を確定していない補足的なオプションであるMurphy分解（\S5.2参照）についても，
同じコード内に実装済みであり，完全較正な合成データ（n=1000）を用いた数値検証では
REL=0.0069，RES=0.0901，UNC=0.2480が得られ，直接計算したBrier Score（0.1650）と
REL−RES+UNC（0.1648）の差は0.000190であり，分位点分割に起因する既知の誤差範囲内で
あることを確認した。
...
パース失敗の記録とJSON形式でのサマリー出力（ECE/Brier/AUROC，および採否未確定のMurphy分解を含む）までを実装済み。
...
\subsection{Murphy分解（探索的オプション）の扱い}

研究の方針の主軸はあくまでECE・Brier Score・AUROCの3指標であり，Murphy分解の採用は
現時点で確定していない。本実験のデータが十分に得られた場合にのみ，補足的な考察として
以下の分解を検討する。
...
ここで$\text{REL} = \sum_k \frac{n_k}{n}(\bar{f}_k - \bar{o}_k)^2$（較正誤差，低いほど良い），
$\text{RES} = \sum_k \frac{n_k}{n}(\bar{o}_k - \bar{o})^2$（識別能力，高いほど良い），
$\text{UNC} = \bar{o}(1-\bar{o})$（データ固有の定数）であり，$n_k$はビン$k$のサンプル数，
$\bar{f}_k$・$\bar{o}_k$はビン$k$内の平均確信度・平均正答率，$\bar{o}$は全体の平均正答率を表す
（Murphy 1973 \cite{murphy1973vector}，Bröcker 2009 \cite{brocker2009reliability} により
任意のProper Scoring Ruleへ一般化済み）。

採否は実験後に以下の基準で判断する：プロンプト条件間でRELとRESが逆方向に動くケースが
あれば採用（機序の説明に使える），RELの順位がECEの順位と完全に一致する，または
RESの順位がAUROCの順位と完全に一致する場合は破棄（既存指標と重複し追加情報がないため）。
