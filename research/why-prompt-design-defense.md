...
> A: Self-consistency は複数回サンプリング（n=10〜40 など）を必要とし、**API コストが 10〜40 倍に増大する**。本研究が目指す「低コストで実用的な改善」の趣旨と相反する。また verbalized confidence との直接比較は Tian 2023 では実施されていない。

### Q3「プロンプト設計で得られる改善は小さいのでは？」

> A: Tian 2023 では内部確率と比べ ECE が約 50% 削減されることが英語環境で確認されている。
> **[2026-07-13 修正]** MlingConf 2025 が日本語含む5言語で verbalized confidence の効果を確認済みのため、
> 「日本語での検証を初めて行う」という主張はできない。本研究の新規性は、
> 複数のプロンプト方式（Verb.1S / Verb.2S / Ling.1S）を日本語ネイティブなデータセットで比較する点、
> すなわち MlingConf が扱っていない軸（方式間比較・翻訳に依らないデータセット）にある。
...
| Xue et al. (2025, ACL Findings) | MlingConf: 日本語含む5言語でverbalized confidenceを検証 | 本研究の新規性の範囲を精緻化する基準（§2.3参照） |
...
4. Tian 2023 が英語で有効性を示した。MlingConf 2025 が日本語含む多言語で単一手法を検証済みだが、
   プロンプト方式間の比較・日本語ネイティブデータセットでの検証は行っていない → 本研究がこの部分を埋める
