...
latexmk 4 パス）．結果は 30 ページ，エラー 0 件，未解決の引用 0 件．
...
引用は `\cite{a}\cite{b}` のように 1 件ずつ分けて出力しており，本文では
**[3][4]** のように並ぶ．`\cite{a,b}` とまとめると [3,4] になるため，
そうしていない．
...
| `title.tex` | 表紙 |
| `abstract.tex` | 中間報告概要 |
| `chapter1.tex` 〜 `chapter6.tex` | 第1章〜第6章 |
| `reference.tex` | 参考文献（20 件） |
| `figure/` | 図 5.1・5.2 |
...
使わない第 7 章の読み込み行はコメントにしてある．また `\markright{参考文献}`
の直前に `\clearpage` を入れた．`\input` は `\include` と違って改ページ
しないため，これが無いと最終章の最後のページの柱が「参考文献」になる．

また，**図目次（`\listoffigures`）と表目次（`\listoftables`）はコメント
にしている．** 図 3 点・表 6 点の中間報告では 1 ページずつ消費するだけで
読み手の役に立たないためである．必要なら `main.tex` の該当行を戻せばよい．
...
- 長い文を含む列がある表（3.2，4.1，6.1）は `tabularx` の `X` 列で折り返す．
  `l` 列は折り返さないため，そのままでは本文幅を超える．
- 列数が多い表 5.1（9 列）と，区間の表記が長い表 5.3 は，テンプレートの流儀に
  合わせて `\scalebox` で縮めている．本文幅に収まることを組版して確認済み．
...
**このフォルダの中身を直接編集しないこと．** `md2tex.py` はこのフォルダを
毎回作り直すため，次に変換したとき消える．この README も
`interim-thesis/overleaf-readme.md` から複製されたものである．
